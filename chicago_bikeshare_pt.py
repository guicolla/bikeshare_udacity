...
#Mostrandoa as 20 primeiras de amostras 
for data in range(1,21):
  print(f'{data}º {data_list[data]}')
...
for gender in range(1,21):
  print(f'{gender}º {data_list[gender][6]}')
...
quantity = []
for i in types:
  quantity.append(' '.join(user_type_list).title().count(i))
y_pos = list(range(len(types)))
plt.bar(y_pos, quantity)
plt.ylabel('Quantidade')
...
answer = "Por que na lista de dados possui registros que o campo genero não está preenchido."
...
#Calulando o max e min trip
for data in trip_duration_list:
  mean_trip += float(data)
  if min_trip == 0. and max_trip == 0.:
    min_trip = float(data)
    max_trip = float(data)
  elif min_trip > float(data):
    min_trip = float(data)
  elif float(data) > max_trip:
    max_trip = float(data)

'''
Calculando a mediana, caso o tamanho da lista seja impar, dividir
a lista por 2, transformar a lista em int e pegar o valor correspondente,
Caso a lista seja par é necessario pegar o index e o index + 1 para realizar
